...
Traditional string based templating engines (`handlebars`, `nunjucks`, many others) have a major drawback in that they blow away any existing dom when anything changes.  This issue has been solved in many existing frameworks (`angular`, `knockout`, `react`) in some really cool ways.  Unfortunaly to take advantage, you need to be using one of these frameworks.  Altr is just a templating engine that does one thing well, and lets you write the rest of your application in your own way.  After initially rendering a template, all subsequent changes only modify the existing dom, element that don't change are not touched and the ones that do are modified rather than replaced.  This gives you more felxability in the way you use event handlers, css transition and much more.
...
## Overview
### Values:
to insert a dynamic value into your template, you can use the `{{ my_value }}` syntax as part of any text content.  You can also use this syntax in any attribute that is not one of the `tags` listed below.  You may also use `altr-attr-my-atribute="{{my_value}}"` to set `my-attribute`.  This is useful for svg which does not permit invalid values in its attributes.

To access an objects property: `{{ my_obj.my_prop }}`.  These lookups are safe, and will simply return an empty string if any part of the lookup fails.
...
And lastly altr supports filters. `{{ my_val -> my_filter(arg) }}`.  altr does not ship with any built in filters, but are descriped in more detail below.
...
```
if the value in this tag is truthy, the element will be rendered as normal.  If it is not, the element will be completly removed from the dom until the value is truthy again.  Child nodes will not be updated while until the value is truthy.

* #### for
```html
<ol altr-for="item in my_items"><li>{{ item.name }}<li></ol>
```
the for tag will take its `innerHtml` and use it as a template to render each item it is passed. When the list of items changes, altr will remove th elements associated with items that have been removed, update the ones that are still part of the list, and create new elements for items that have been added.  By default altr will use indexOf to determin if an item is still part of the list and where it is located.  You can also specify a unique key if you want to pass in objects that represent the same item, but may not be the exact same object.
```html
...
```
the with tag will make any property of the passed value directly accessible in any child nodes.  Values from the partent scope will still be accessable as well.

* #### include
```html
<div altr-include="another_template"></div>
```
the include tag will render another template into its element. You will need to use one of the include methods described below to make the template avaiable.
 ...
 * attr: the attribute that initialized the tag
 * constructor: a function that takes an element and the contents of the specified attrute, and returns an update function.  the tag is responsible for updating its children.
